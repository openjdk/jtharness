...
package com.sun.javatest.functional.suite.testrunner;
...
class DemoTestRunner extends TestRunner {
    ...
    public synchronized boolean runTests(Iterator<TestDescription> testIter)
        ...
        System.err.println("DemoTestRunner.runTests method called");
            ...
            while (!stopping) {
                for (int i = 0; i < threads.length; i++) {
                    ...
                    if (t == null || !activeThreads.contains(t)) {
                        ...
                        t.setName("DemoTestRunner:Worker-" + i + ":" + n++);
                        t.start();
                        t.setPriority(prio);
                        activeThreads.add(t);
                        threads[i] = t;
    ...
    private TestResult createErrorResult(TestDescription td, String reason, Throwable t) { // make more i18n
        ...
        try {
            ...
            workDir.log(i18n, "dtr.unexpectedThrowable", e, EXCEPTION);
        }
        return tr;
    }
